...
def LossNCSN(model, x, sigmas, return_per_sigma=False):
    ...
    # sum of losses per level + counter per level (detach -> no grad)
...
def get_args():
    parser = argparse.ArgumentParser(
        description="Score-based NCSN model on MNIST (train only)"
    )
    ...
    parser.add_argument("--exp-name", type=str, default="mnist_ncsn_debug")
...
def main(args):
    ...
    print("MNIST train size:", len(training_data))
    ...
    print(f"Model: in_ch={in_ch}, base_ch={base_ch}, channel_mults={channel_mults}")
    ...
    # running loss per sigma (sur la fenêtre EVAL_EVERY)
    loss_sigma_running = torch.zeros(n_sigmas)
    count_sigma_running = torch.zeros(n_sigmas)
    L_sigma_history = []   # list of vectors (n_sigmas,) at each eval
    ...
    for epoch in range(N_EPOCH):
        for u, (x, _) in enumerate(dataloader):
            ...
            if (step) % EVAL_EVERY == 0:
                ...
                # mean loss per sigma over window
